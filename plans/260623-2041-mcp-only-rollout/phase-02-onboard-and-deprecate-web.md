...
- [x] Branch `legacy-web` checkpoint từ `origin/master` @ `b343028` (snapshot trước decommission, pushed remote)
- [x] **Comment block** web/agent trong `infra/docker-compose.yml` với DECOMMISSIONED header (giữ YAML inline)
...
- [ ] **Resurrect drill thật**: SSH sandbox/clone fresh → `infra/scripts/resurrect-drill.sh` → confirm <30 phút bootable, log vào `infra/RESURRECT-NOTES.md`
- [ ] Pin lockfiles (`web/package-lock.json`, `agent/requirements.txt`) trên branch `legacy-web` nếu drill cần
